...
const GetColumnSearchProps = (dataIndex: string, name: string, customRender?: (render: any) => any): object => {
  ...
  return {
    filterDropdown: (props: { setSelectedKeys: (value: string[]) => void; selectedKeys: string[]; confirm: () => void; clearFilters: () => void }) => {
    ...
    onFilter: (value: any, record: any) => {
      return customRender ? customRender(record[dataIndex])?.toString()?.toLowerCase()?.includes(value?.toLowerCase()) : record[dataIndex]?.toString()?.toLowerCase()?.includes(value?.toLowerCase())
    },
    render: (text: any) => {
